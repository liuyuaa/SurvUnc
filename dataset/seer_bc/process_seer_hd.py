...
df = df[df["COD to site recode"].isin(['Diseases of Heart', 'Alive'])]

df = pd.concat([
    df[cat_cols], df[num_cols], df["duration"], df["event_heart"], df["event_breast"]
    ], axis=1)

df.to_csv("seer_format_hd.csv", index=False)
print("done first")

from sklearn.preprocessing import KBinsDiscretizer, LabelEncoder, StandardScaler

df = pd.read_csv("seer_format_hd.csv")
x_df_cat = {}
for col in cat_cols:
...
df.to_csv("seer_processed_hd.csv", index=False)
