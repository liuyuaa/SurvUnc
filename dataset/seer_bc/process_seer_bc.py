...
# Obtain this file by executing the seer code with database. See SurvTRACE for more details.
# https://github.com/RyanWangZf/SurvTRACE

df = pd.read_csv("41w_2000_2019_2021_17_register.csv")
...
df = df[df["COD to site recode"].isin(['Breast', 'Alive'])] 

df = pd.concat([df[cat_cols], df[num_cols], df["duration"], df["event_heart"], df["event_breast"]], axis=1)

df.to_csv("seer_format_bc.csv", index=False)
print("done first")

from sklearn.preprocessing import KBinsDiscretizer, LabelEncoder, StandardScaler

df = pd.read_csv("seer_format_bc.csv")
x_df_cat = {}
for col in cat_cols:
...
df = pd.concat([x_df_cat, x_df_num, df["duration"], df["event_heart"], df["event_breast"]], axis=1)
